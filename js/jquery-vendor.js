...
(function($) {
	...
	$.event.special.mousewheel = {
	    setup: function() {
	        if ( this.addEventListener && !$.browser.msie) {	        		        	
	            for ( var i=types.length; i; ) {
	                this.addEventListener( types[--i], handler, false );
	            }
	        } else if($.browser.msie) {
	        	elemIE = this;
	        	document.attachEvent('onmousewheel', handlerIE, false);
	        } else {
	...
	var elemIE;
	/**
	 * IE adjustments by R. van der Ark 2012
 	 * @param {Object} event original event object
	 */
	function handlerIE(event) {		
	    var args = [].slice.call( arguments, 1 ), delta = 0, returnValue = true, deltaX = 0, deltaY = 0;
		
		var box = {
			y0: $(elemIE).offset().top,
			x0:	$(elemIE).offset().left,
			y1:	$(elemIE).offset().top + $(elemIE).height(),
			x1: $(elemIE).offset().left + $(elemIE).width()
		};
	
		if(event.clientX > box.x0 && event.clientX < box.x1 && event.clientY > box.y0 && event.clientY < box.y1) {
			var orgEvent = event;
			...
			return ($.event.dispatch || $.event.handle).apply(elemIE, args)
		}
		return true;
	}
